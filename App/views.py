...
#             refresh = RefreshToken.for_user(user)
#             return Response({
#                 'success': True,  # أضفنا success عشان يتطابق مع Flutter
#                 'token': str(refresh.access_token),
#                 'user_id': user.id,
#                 'message': 'Login successful'
#             }, status=status.HTTP_200_OK)
...
#     def post(self, request):
#         print("Received data:", request.data)
#         # نستخدم NurserySerializer مباشرة بدل ما نعمل Parent الأول
#         nursery_data = {
#             'name': request.data.get('full_name'),  # Flutter بيبعت full_name
#             'email': request.data.get('email'),
#             'phone': request.data.get('phone_number'),
#             'password': request.data.get('password'),
#             'location': request.data.get('location', 'Unknown'),
...
#                     'parent_id': nursery.id,  # أعدلنا عشان يرجع parent_id زي ما Flutter متوقع
#                 }, status=status.HTTP_201_CREATED)
#             except Exception as e:
#                 return Response({'error': f'Failed to create nursery: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
#         print("Serializer errors:", serializer.errors)
...
#                     'token': str(refresh.access_token),
#                     'user_id': nursery.user.id,
#                     'parent_id': nursery.id,  # بنرجع parent_id زي ما Flutter متوقع
#                 }, status=status.HTTP_201_CREATED)
#             except Exception as e:
#                 return Response({'error': f'Failed to create nursery: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
#         print("Serializer errors:", serializer.errors)
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            refresh = RefreshToken.for_user(user)
            # نحدد نوع المستخدم بناءً على user_type
...
# إزالة login_parent_view و login_nursery_view لأننا هنديهم كل حاجة في login_view
