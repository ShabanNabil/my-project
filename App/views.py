...
class SignUpView(APIView):
    ...
    def post(self, request):
        ...
        if user_serializer.is_valid():
            try:
                ...
                if parent_serializer.is_valid():
                    ...
                    user.delete()
                    return Response({'error': 'Failed to create parent', 'details': parent_serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'error': 'Failed to create user', 'details': user_serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        if user_serializer.is_valid():
            try:
                ...
                return Response({'error': f'Invalid longitude or latitude: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
            except Exception as e:
...
def get_nurseries(request):
    nurseries = Nursery.objects.filter(status='accepted')  # فقط الحضانات المقبولة
    serializer = NurserySerializer(nurseries, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)

# باقي الكلاسات (NurserySignUpView, ParentResetPasswordView, ViewSets, etc.)
@api_view(['POST'])
@permission_classes([AllowAny])
@csrf_exempt
